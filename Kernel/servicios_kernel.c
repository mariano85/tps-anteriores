...
void interrupcion(t_process* aProcess, int32_t dir_memoria){
	...
	if(header == CPU_TO_KERNEL_IMPRIMIR_TEXTO){ // CAMBIAR POR CPU_TO_KRN_INTERRUPCION
...
void bloquear(t_process* aProcess, int32_t id){
		...
		t_contenido mensaje;
		t_header header = recibirMensaje(socketCPU, mensaje, logKernel);
		if(header == CPU_TO_KERNEL_IMPRIMIR_TEXTO){ // CAMBIAR POR CPU_TO_KRN_BLOQUEAR
...
void crear_hilo(t_process* aProcess){
	...
	t_contenido mensaje;
			t_header header = recibirMensaje(socketCPU, mensaje, logKernel);
			if(header == CPU_TO_KERNEL_IMPRIMIR_TEXTO){ // CAMBIAR POR CPU_TO_KRN_CREAR_HILO
